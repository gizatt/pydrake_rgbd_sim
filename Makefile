...
bash:
	xhost +local:root
	$(DOCKER) run -it -v $(SRC)/pydrake_rgbd_sim:/src/workspace --env KERAS_BACKEND=$(BACKEND) --net="host" -e DISPLAY -e QT_X11_NO_MITSHM=1 -v /tmp/.X11-unix:/tmp/.X11-unix:rw $(IMAGE_NAME) bash
